...
public final class RectangleBoard extends NMBoardBase {

    public RectangleBoard(int n, int m, int k) {
        super(n, m, k);
    }

    @Override
    protected boolean checkWin(Move move) {
        if (goDown(move) + goUp(move) + 1 >= k)
            return true;
    ...
    private int goDown(Move move) {
        int sum = 0;

        for (int i = move.getRow() + 1; i < n && field[i][move.getCol()] == move.getValue() && sum < k; ++i) {
            sum++;
        }
        return sum;
    }
    ...
    private int goUp(Move move) {
        int sum = 0;

        for (int i = move.getRow() - 1; i >= 0 && field[i][move.getCol()] == move.getValue() && sum < k; --i) {
            sum++;
        }
        return sum;
    }
