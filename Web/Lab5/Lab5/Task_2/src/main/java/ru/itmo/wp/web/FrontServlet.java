...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            method = Arrays.stream(clazz.getDeclaredMethods())
                    .filter(x -> x.getName().equals(route.getAction()) && Arrays.stream(x.getParameterTypes())
                            .allMatch(type -> type.equals(HttpServletRequest.class) || type.equals(Map.class)))
                    .findFirst()
                    .orElse(null);
        }
        ...
        if (language != null && language.length() == 2 && language.matches("\\w+")
        ...
        Template template = newTemplate(pageClass.getSimpleName() + ".ftlh", language);
    ...
    private Template newTemplate(String templateName, String language) throws ServletException {
        ...
        if (sourceConfiguration != null) {
            try {
                template = sourceConfiguration.getTemplate(templateName, Locale.forLanguageTag(language));
        ...
        if (template == null && targetConfiguration != null) {
            try {
                template = targetConfiguration.getTemplate(templateName, Locale.forLanguageTag(language));
            } catch (TemplateNotFoundException ignored) {
                // No operations.
            } catch (IOException e) {
                throw new ServletException("Can't load template [templateName=" + templateName + "]", e);
