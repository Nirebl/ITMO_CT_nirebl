...
public class TalksRepositoryImpl implements TalksRepository {
    ...
    public List<Talk> getUserTalks(long userId) {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM `Talk` WHERE sourceUserId = ? or targetUserId = ? ORDER BY id DESC")) {
            ...
            throw new RepositoryException("Can't find Talk for User.", e);
        }
        return talks;
    }
