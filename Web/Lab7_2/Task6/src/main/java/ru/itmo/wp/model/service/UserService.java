...
public class UserService {
    ...
    public void validateUserId(long id) throws ValidationException
    {
        User dbUser = userRepository.find(id);
        if(dbUser == null)
            throw new ValidationException("userId is incorrect");
    }
    ...
    public void updateRule(long id, long updatedUserid, boolean setRule) throws ValidationException {
        User admin = find(id);

        if(!admin.isAdmin())
            throw new ValidationException("You are not a admin.");

        userRepository.updateRule(updatedUserid, setRule);
    }
}
